  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: CustomText(text: widget.productModel.category??"",size: 30,),
        centerTitle: true,
        elevation: 0,
      ),
       body: Padding(
                ...
                children: [
                  Center(
                    child: Container(
                      ...
                      width: Get.width/1.6,
                      color: Colors.black,
                      child: CustomNetworkImage(image: widget.productModel.image??"",),),
                  ),
                  IconButton(onPressed: (){}, icon: const Icon(Icons.favorite_border,color: Colors.red,size: 30,))
                ],
              ),
               const SizedBox(height: 10,),
               CustomText(text: "Product Name : \n${widget.productModel.title??""}",maxLines: 4,),
               const SizedBox(height: 10,),
               Row(
                 mainAxisAlignment: MainAxisAlignment.spaceBetween,
                 children: [
                   CustomText(text: "Stock : ${widget.productModel.rating!.count??""}"),
                   Row(
                     children: [
                       CustomText(text: "Price : "),
                       CustomText(text: "\$${widget.productModel.price??""}",color: AppColor.primaryColor,),
                     ],
                   ),
                 ],
               ),
               const SizedBox(height: 10,),
               CustomText(text: "Description : \n${widget.productModel.description??""}",maxLines: 20,),
               const SizedBox(height: 10,),
